 ...
 * Description: If file is not ELF file - 98.
 */
 ...
 * Description: Magic numbers to be separated with spaces.
 */
...
void print_magic(unsigned char *e_ident)
{
	...
	for (f1 = 0; f1 < EI_NIDENT; f1++)
	{
		printf("%02x", e_ident[f1]);
 ...
 * Description: If the file cannot be closed - 98.
 */
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	dy = read(dx, header, sizeof(Elf64_Ehdr));
	if (dy == -1)
	{
		free(header);
		close_elf(dx);
